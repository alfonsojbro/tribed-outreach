...
Same gift-first move as the DMs, but this is a real inbox, not a chat. It earns deletion just as fast as a DM earns a scroll-past, so the craft is tighter. This mode powers cold email openers and the daily pipeline's email drafts (kind "email", sent through Instantly after approval). Adapted from Corey Haines' cold-email method, kept in Tribed's voice; where the two conflict, the Tribed core (gift-first, no banned phrases, no call ask pre-demo) wins.
...
- Avoid PAS/AIDA-style agitation: manufacturing a pain point clashes with a gift.
