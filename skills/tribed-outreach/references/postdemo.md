...
Booking mechanics (rewritten 2026-09-13, tightened 2026-09-14): the ask is two concrete times, stated in the prospect's own timezone, plus the recorded tour for the call-averse. The scheduling link is not in the yes message at all: it appears once, as the last line of the 48-hour bump, for the prospect who took neither pair. On 2026-09-13 the `digital_university` funnel read `interested` 112, `meeting` 5, `won` 3: every win came through a meeting and 95% of warm replies never reached one, and a bare link is where they died (see "The yes" below for the mechanics). The booking link is one fixed URL, `https://tribed.io/book` (Alfonso, 2026-09-21), and it is written ONLY as the last line of the 48-hour bump: runs send unattended since 2026-09-17, so there is no human left to drop a placeholder in, and on 2026-09-21 a run improvised `tribed.io/c/digital-university/book` from the outreach accountId. That page is RETIRED as of 2026-09-21: `https://tribed.io/book` is the only booking link for everything, podcast recordings included, so the old url goes to nobody. Never build a `/c/<account>/book` url. [tour link] stays a placeholder. Offer the call once per reply at most, and only once the commitment gate is passed. If they decline the call twice, stop offering it and answer their questions in the DM instead, a forced call loses the deal that patience keeps.
...
- **One page, one config.** The page every prospect and podcast guest books on is `https://tribed.io/book`, and it runs on the `tribed` booking config, so read THAT config for slot arithmetic: `get_booking_config({ communityId: "tribed" })`. The `digital_university` config named in the bullet above is the retired podcast page (Alfonso, 2026-09-21); its hours and the worked overlap table further down were computed from it and are stale until recomputed from the `tribed` config. When subtracting taken slots, check confirmed bookings on BOTH communities while the old page still holds live bookings.
...
**"I need to think about it." / "let me digest" / "I'll get back to you".** That is a request for space, not a booking signal. Release pressure completely and STOP: no call ask and no link in that reply (Alfonso, 2026-09-21, on a reply that offered 15 minutes to a prospect who had asked for a couple of days: "he didnt ask for a call?"). If the same message carries a direct question, answer it in the message, in the few words they asked for. "Easier to talk through than to type" is a deflection and reads as evasion. For "how would we do business" the structure is sayable without the number (references/answers.md, entry 1): it stays their brand and their members, a flat monthly fee, setup free, month to month, Tribed builds and runs the infrastructure and the AI. The call is offered when THEY reopen the thread, as two concrete times. If they go quiet, the post-demo ladder owns the silence.
