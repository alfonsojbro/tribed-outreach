...
- **The link and the slot source are different things.** The page a prospect books on is `https://tribed.io/book`. The config read above is only the arithmetic for proposing two times, and a proposed time is a proposal until Alfonso confirms it. OPEN, for Alfonso (flagged 2026-09-21): the `digital_university` config (Mon to Fri 07:00 to 17:00, 24h notice) and the `tribed` config (Mon to Sat 09:00 to 17:00, 4h notice) disagree, and `tribed.io/book` runs on neither exactly, so a 07:00 to 09:00 Makassar slot offered in a DM may not be bookable on the page. Until he reconciles them, prefer slots inside BOTH windows.
