...
**Sending the InMail closes the lead.** `li_state` goes to `"done"`, `nextActionAt` clears, the stage stays. The drip does not poll it afterwards. A reply lands in the SALES NAVIGATOR inbox, not `/messaging/` — the daily run reads it with `get_sales_inbox({ inmail_filter: "ALL", limit: 500 })` and `get_sales_conversation({ thread_id })` on every matched row (the parameter is `inmail_filter`, never `filter`; `limit` takes 1-500 and the paged reader walks the list, 276 threads in 50s on 2026-09-06). Never read the ACCEPTED bucket for this: it returned 0 rows on 2026-09-12, 09-14 and 09-15 while ALL listed unanswered replies, and a row carries no message direction, so only the opened thread says who wrote last, and answers INTO THE SAME SN THREAD with `send_sales_message` (dry run with `confirm_send: false` first, Alfonso approves the copy, then `confirm_send: true`; a reply spends no InMail credit). Never answer an InMail reply with a new InMail, and never conclude "no reply" from `/messaging/` reads. Full contract in pipeline.md, "The InMail escalation". Known gap, accepted by Alfonso: a silent ACCEPTANCE after the InMail (no message) is not noticed and follow-up 1 never fires.
