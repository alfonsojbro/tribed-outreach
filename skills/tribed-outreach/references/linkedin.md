...
One InMail, ever, authored the day it ships, day 3 to day 21. Copy it like 4B. **Read the thread and the note we sent first** (`data.li_note` is on the record): if the note already promised a demo, the InMail delivers the link (`data.demo_tracked_url`, never the bare `data.demo_url` — see pipeline.md, "the only link you SEND") instead of teasing a second time. Where no demo exists, use the 4B tease.

**Sending the InMail closes the lead.** `li_state` goes to `"done"`, `nextActionAt` clears, the stage stays. The drip does not poll it afterwards. A reply lands in the SALES NAVIGATOR inbox, not `/messaging/` — the daily run reads it with `get_sales_inbox({ inmail_filter: "ACCEPTED", limit: 500 })` and `get_sales_conversation({ thread_id })` (the parameter is `inmail_filter`, never `filter`; `limit` takes 1-500 and the paged reader returns the whole bucket, 276 ACCEPTED threads in 50s on 2026-09-06), and answers INTO THE SAME SN THREAD with `send_sales_message` (dry run with `confirm_send: false` first, Alfonso approves the copy, then `confirm_send: true`; a reply spends no InMail credit). Never answer an InMail reply with a new InMail, and never conclude "no reply" from `/messaging/` reads. The post-demo ladder is the exception to this close: when the InMail delivered a demo, the lead is enrolled on the ladder and the founder drip re-opens that same SN thread for the ladder's three bumps. Full contract in pipeline.md, "The InMail escalation". Known gap, accepted by Alfonso: a silent ACCEPTANCE after the InMail (no message) is not noticed and follow-up 1 never fires.
