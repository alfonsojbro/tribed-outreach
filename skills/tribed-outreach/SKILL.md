...
No em dashes. No bullet points or lists inside a message. No call ask before the prospect has seen the demo (Modes 1-4, 6); after they've seen it, the walkthrough call is the goal and Mode 5 owns it. No false urgency, no guilt. Assume the follow or connection exists except in LinkedIn connection notes, and even there never say "I'd love to connect". One CTA, phrased as a question, at the end (connection notes: no explicit CTA). Never write the demo link (app.tribed.io/[handle]) or the tour video link; leave [demo link] or [tour link] placeholders and the user drops them in. The booking link is the one exception because runs send unattended: it is always `https://tribed.io/book`, the ONE booking link for everything, sales calls and podcast recordings alike (Alfonso, 2026-09-21). In a sales thread it appears only in the 48-hour bump. `tribed.io/c/digital-university/book` is retired and goes to nobody. After a post-demo yes the ask is two concrete times in the prospect's timezone, read off the booking config, in a message under 45 words; the booking link appears only in the 48-hour bump, never in the yes, and the price stays held back (Mode 5). A prospect who asks for time ("let me digest", "I'll get back to you") gets the time and a straight answer to anything they asked, with no call ask and no link in that reply (Mode 5). A decline is never just closed out: it gets one referral ask in the same message as the graceful exit, never as a separate message afterwards, and never when the no is a boundary (opt-out, complaint, legal or trademark matter, or a request to stop). See references/replies.md.
